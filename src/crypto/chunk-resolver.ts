...
export class ChunkResolver {
  ...
  get encryptChunkSize(): number {
    const found = encryptions.find(e => e.encryptionType === this.encryptionType).encryptChunkSize;
    return found ? found : encryptions[0].encryptChunkSize;
  }

  get decryptChunkSize(): number {
    const found = encryptions.find(e => e.encryptionType === this.encryptionType).decryptChunkSize;
